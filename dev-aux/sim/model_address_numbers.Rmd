...
my_hist(house_numbers, x_label = "House numbers", title = title)
```
...
title <- paste("Distribution of house numbers", min(x), "to", max(x), "(actual)")
my_hist(x, x_label = "House numbers", title = title)
```
...
my_hist(samp, x_label = "House numbers", title = title)
```
...
```{r fig.width=11.53}
# Model house numbers
samp <- etmstuff:::sim_distribution(
...
```{r}
# Summarize `location` types
df_summarize_location_type <- df_units %>%
  group_by(location_type) %>%
  count() %>%
  mutate(pct = etmstuff::pct(n, nrow(df_units)))
my_kable(df_summarize_location_type)
```

```{r}
# View `location_type` "other"
df <- df_units %>%
...
```{r}
df_summarize_location_number <- df_units %>%
  group_by(nchar(loc_number)) %>%
  count() %>%
  mutate(pct = etmstuff::pct(n, nrow(df_units)))
my_kable(df_summarize_location_number)
```

```{r}
df_summarize_location_letter <- df_units %>%
  filter(location_type != "number")
df_summarize_location_letter <- df_summarize_location_letter %>%
  group_by(loc_letter) %>%
  count() %>%
  mutate(pct = etmstuff::pct(n, nrow(df_summarize_location_letter)))
my_kable(df_summarize_location_letter, scroll_box_ht = "600px")
```
...
my_hist(unit_numbers, x_label = "Unit numbers", title = title)
```
...
my_hist(x, x_label = "Unit numbers", title = title)
```
...
my_hist(samp, x_label = "Unit numbers", title = title)
```
