...
#'
#' @description
#' This function determines the confirmed status (`confirmed`, `unconfirmed`, `cap_after_ven`, `unknown_early`, or `unknown`) for each test in df.
#'
#' @details
#' Tests are subset into sequences for each person in the data set and sorted by 1) collection date, 2) specimen source (capillary tests before venous tests), and 3) result value (in descending order). Within a test sequence, confirmed status values are assigned as follows:
#'
...
#'
#' Other variables returned are `test_gap` (the number of days since the previous test in the sequence) and `test_reason`, which has the following values:
#'
...
#'
#' @return The input dataframe with the new variables `test_gap` (in days since the previous test), either `confirmed_status_cdc` or `confirmed_status_tracking`, and `test_reason`.
#' @export
#'
...
assign_confirmed_status <- function(df, row_id, rule = c("cdc", "tracking"), start_date, silent = FALSE) {
  ...
  for (i in 1:length(person_id_unq)) {
    ...
    # Get test gap, prior test source, & prior test elevated status
    if (n == 1) {
      test_seq$test_gap <- NA
      test_seq$pr_src <- NA
      test_seq$pr_elev <- NA
    } else {
      dates <- test_seq$collection_date
      days <- as.numeric(dates[2:n] - dates[1:(n - 1)])
      test_seq$test_gap <- c(NA, days)
    ...
    test_seq <- test_seq |>
      dplyr::mutate({{ var_cfm }} := dplyr::case_when(
        specimen_source == "Blood - venous" ~
          "confirmed",
        specimen_source == "Blood - capillary" & is.na(test_gap) &
          collection_date < period_cfm_unk ~
          "unknown_early",
        specimen_source == "Blood - capillary" & test_gap <= cfm_retest_wndw &
        ...
        specimen_source == "Blood - capillary" &
          (test_gap > cfm_retest_wndw | is.na(test_gap)) ~
          "unconfirmed",
        TRUE ~
          "unknown"
      ))
    ...
    test_seq <- test_seq |>
      dplyr::mutate(test_reason = dplyr::case_when(
        # A screening test is an unconfirmed capillary test with no prior test within one year
        .data[[var_cfm]] == "unconfirmed" & collection_date >= period_scrn_unk &
          (test_gap > 365 | is.na(test_gap)) ~
          "screening",
        TRUE ~ NA
      ))
