...
#'
#' Replace the dirty address values in `df$var` with the cleaned values in `source$replacement_text` generated by [clean_street_address()].
#'
...
replace_values <- function(df, var, row_id = "address_id", source) {
  ...
  df_sub <- df %>%
    dplyr::semi_join(source, by = row_id)

  # Remove rows in `df` that match cleaned rows in `source`
  df <- df %>%
    dplyr::anti_join(source, by = row_id)
