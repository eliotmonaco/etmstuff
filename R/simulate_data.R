...
#'
#' Create a dataframe of simulated addresses. Street names are from `common_streets`, which is based on a list of the most common street names in the US. City and zip code pairs are from `ks_cities`, which is the reference list used to check cities in [validate_values()]. Other elements (e.g., house numbers, directions, street suffixes, and units) are generated pseudorandomly using `sample()`. By setting `dirty = TRUE`, the function will alter values in `street`, `city`, and `zip` for the purpose of testing [validate_values()] and [clean_values()].
#'
...
#' @export
#'
#' @importFrom stringr str_to_title str_sub<-
#'
#' @examples
...
simulate_data <- function(rows, dirty=FALSE) {
  ...
  if (dirty) {
    ...
    pobox <- paste(sample(pobox, n, replace = T), sample(10:9999, n, replace = T))
    ...
    n <- ifelse(n < 2, 2, n)
    n <- sample(1:rows, n)
    df$city[n] <- sapply(df$city[n], string_delete, simplify = T)

    # Misspell `city`: string_add()
    n <- rows %/% 100
    n <- ifelse(n < 2, 2, n)
    n <- sample(1:rows, n)
    df$city[n] <- sapply(df$city[n], string_add, simplify = T)
